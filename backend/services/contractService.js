...
function init() {
  ...
  try {
    // Use static network to avoid batch network-detection requests on startup
    provider = new ethers.JsonRpcProvider(process.env.RPC_URL, {
      chainId: 80002,
      name: "amoy"
    }, { staticNetwork: true, polling: true, pollingInterval: 12000 });
    wallet = new ethers.Wallet(process.env.PRIVATE_KEY, provider);
    contract = new ethers.Contract(process.env.CONTRACT_ADDRESS, contractABI, wallet);
    return contract;
  } catch (err) {
    // Non-blocking: log one warning and return null — app continues normally
    console.warn("ContractService: Provider initialization failed —", err.message);
    return null;
  }
}
...
async function createDrug(drugID, name, batchNumber, expiryTimestamp) {
  const c = init();
  if (!c) throw new Error("Blockchain unavailable");
